...
- **Netplan:** **`write_files`** → **`/etc/cloud/cloud.cfg.d/99-lab-disable-cloud-init-network.cfg`**, then **`/etc/netplan/50-cloud-init.yaml`** with static **`ens2`**, default via **10.30.0.1** (jump) or **10.10.0.1** / **10.20.0.1** (cluster). **Cluster nodes:** **`nameservers: 10.30.0.10`** only. **jump-1:** **`10.30.0.10`** then **`198.18.133.1`** for bootstrap before `dnsmasq` is installed. **`runcmd`** removes legacy **`99-*-static.yaml`**, then **`netplan apply`**.
