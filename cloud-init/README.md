...
- **Netplan:** **`write_files`** → **`/etc/cloud/cloud.cfg.d/99-lab-disable-cloud-init-network.cfg`** (`network: {config: disabled}` so cloud-init does not emit a conflicting DHCP **`50-cloud-init.yaml`**), then **`/etc/netplan/50-cloud-init.yaml`** with static **`ens2`**, default via **10.30.0.1** (jump) or **10.10.0.1** / **10.20.0.1** (cluster), **`nameservers: 198.18.133.1`** (host environment DNS). **`runcmd`** removes legacy **`99-*-static.yaml`**, then **`netplan apply`**.
